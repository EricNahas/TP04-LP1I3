    ...
    TP04 (String titulo, int largura, int altura, int colinic, int lininic)
    {
		...
		if (e.getSource() == b1) 
		{
			try
			{
				...
				ResultSet rs;
				rs = st.executeQuery("SELECT nome_func, sal_func, ds_cargo FROM tbfuncs JOIN tbcargos on cd_cargo = cod_cargo WHERE nome_func LIKE '%" + t1.getText() + "%'");
		...
		else if (e.getSource() == b2) 
		{
			try
			{
				...
				ResultSet rs;
				rs = st.executeQuery("SELECT nome_func, sal_func, ds_cargo FROM tbfuncs JOIN tbcargos on cd_cargo = cod_cargo");
